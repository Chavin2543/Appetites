...
struct PostDetailsView: View {
    ...
    var body: some View {
                ...
                ZStack {
                    VStack (spacing: 0 ) {
                        Button {
                            ...
                            WebImage(url: URL(string: post.photoLinksList?.first ?? "https://images.unsplash.com/photo-1607434472257-d9f8e57a643d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1772&q=80"))
                                .resizable()
                                .scaledToFill()
                        ...
                        VStack (alignment:.leading) {
                            ...
                            Text("\(post.postCaption ?? "Can't Retrieve")")
                                .font(.caption)
                        ...
                        ScrollView(showsIndicators:false) {
                            VStack {
                                ...
                                HStack {
                                    HStack {
                                        WebImage(url: URL(string: userService.user.profilePictureLink ?? ""))
                                            .resizable()
                                            .scaledToFill()
                    ...
                    .sheet(isPresented: $isViewingFullPic) {
                        WebImage(url: URL(string : post.photoLinksList?.first ?? "https://images.pexels.com/photos/10808064/pexels-photo-10808064.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260"))
                            .resizable()
                            .scaledToFit()
        ...
        .onAppear {
            vm.getPostDetail(token: userService.token, postID: "\(post.postID)")
        }
    }
}
